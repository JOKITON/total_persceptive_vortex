...
class EEGData:
	...
	def filter_data(self, tmax=None, verbose=False):
		...
		self.raw_filt_h = self.raw_h.copy().filter(l_freq=self.l_freq, h_freq=self.h_freq, fir_design="firwin", skip_by_annotation="edge", verbose=False)
		self.raw_filt_hf = self.raw_hf.copy().filter(l_freq=self.l_freq, h_freq=self.h_freq, fir_design="firwin", skip_by_annotation="edge", verbose=False)
	...
	def crt_epochs(self, data, events, event_dict, group_type, verbose=False):
		...
		if group_type == 'hands':
			...
			freq_bands = self.csp_config["freq_bands__03"]
		else:
	...
	def csp(self, data, labels, freq_bands, epochs_info, verbose=False):
		...
		print(n_components, fs, freq_bands)
	...
	def train_model(self, X, y):
		...
		print("Train Accuracy [ LGBM ]:", accuracy_score(y, self.lgb_clf.predict(X)))
	...
	def pred(self, X, y, n_preds=20, prt_matrix=False):
		...
		print("Test Accuracy [ LDA ]:", accuracy_score(y, y_pred_lda))
		print("Test Accuracy [ SVM ]:", accuracy_score(y, y_pred_svm))
		print("Test Accuracy [ RF ]:", accuracy_score(y, y_pred_rf))
		print("Test Accuracy [ LGB ]:\n", accuracy_score(y, y_pred_lgb))
